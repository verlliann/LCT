document.addEventListener("DOMContentLoaded", function() {
    ...
    function updateWebLinkPosition() {
        // Получаем контейнер, в котором находятся videoplayer элементы
        let videoContainer = document.getElementById('video-placeholder');
        // Вычисляем новую позицию для weblink
        let newTop = videoContainer.offsetHeight + 20; // 20px отступ от последнего videoplayer
        // Обновляем позицию weblink
        let webLink = document.getElementById('webLinkdButton');
        webLink.style.top = newTop + 'px';
    }
